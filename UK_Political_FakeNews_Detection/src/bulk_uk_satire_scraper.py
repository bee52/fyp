...
# Target UK Satire Sites (Proxies for "Fake" News)
SATIRE_TARGETS = [
    {'name': 'DailyMash', 'url': 'https://www.thedailymash.co.uk'},
    {'name': 'NewsThump', 'url': 'https://newsthump.com'},
    {'name': 'NewsBiscuit', 'url': 'http://www.newsbiscuit.com'}
]
...
def scrape_satire(source, max_articles=100, base_delay=2.0, max_retries=3):
    ...
    for article in paper.articles:
        ...
        try:
            ...
            if scrape_with_retry(article, max_retries=max_retries):
                # Filter: minimal length to avoid error pages or cookie banners
                if len(article.text) > 200: 
                    articles_data.append(normalize_article_record({
                        'title': article.title,
                        'text': article.text,
                        'source': source['name'],
                        'date': datetime.now().strftime("%Y-%m-%d"),
                        'label': 1  # 1 = FAKE (Satire)
                    }))
                    success_count += 1
                    print(f"[{success_count}] Collected: {article.title[:40]}...")
            else:
                failed_count += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scrape UK satire/fake-news sources")
...
def main():
    ...
    all_articles = []
    for source in SATIRE_TARGETS:
        data = scrape_satire(source, max_articles=max_articles, base_delay=base_delay, max_retries=max_retries)
    ...
    filename = out_dir / f"uk_fake_satire_{datetime.now().strftime('%Y%m%d')}.csv"
    df = df[["title", "text", "source", "date", "label"]]
    df.to_csv(filename, index=False)
    
    logger.info(f"SUCCESS: Saved {len(df)} Satire articles to {filename}")
    print(f"\nSUCCESS: Saved {len(df)} Satire articles to {filename}")
    print("Next Step: Merge this with your 'Real' CSV to create the training set.")
